...
def OddMagicSquare(n):
	...
	print(colored("\nMagic Square for %s order" %(n), "light_green"))
	print(colored("------------------------", "light_green"))
	for i in range(0, n):
		for j in range(0, n):
			print(colored(magic_square[i][j], "light_green"), end=' ')
...
size = int(input("Enter the Magic Square size (it's must be ODD) : "))
...
while size != 0:
    ...
    OddMagicSquare(size)
    sleep(1)
    size = int(input("\nEnter the Magic Square size (it's must be ODD, enter 0 if you want to exit) : "))
